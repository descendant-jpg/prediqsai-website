...
import PremiumPlanCta from "@/components/site/PremiumPlanCta";
...
function Plan({name,price,suffix,items,featured=false}:{name:string;price:string;suffix?:string;items:string[];featured?:boolean}) { return <section className={`rounded-2xl border p-8 ${featured ? "border-volt bg-panel-2 card-glow" : "border-edge bg-panel"}`}><p className="font-mono text-xs uppercase tracking-[0.2em] text-volt">{name}</p><p className="mt-6 font-display text-4xl font-bold">{price}<span className="font-body text-base font-normal text-muted">{suffix}</span></p><ul className="mt-7 space-y-3 text-sm text-muted">{items.map(item=><li key={item} className="flex gap-3"><span className="text-volt">✓</span>{item}</li>)}</ul>{featured ? <PremiumPlanCta compact /> : <AcquisitionCta kind="free" className="mt-8 block rounded-full border border-edge px-4 py-3 text-center text-sm font-semibold text-ice">Start free</AcquisitionCta>}</section> }
